...
class Account {
    ...
    constructor(accountObj) { // accountObj that is used everywhere
    ...
    static checkCredentials(accountObj) { // this method checks whether or not the input exists on the database.
            ...
            (async () => {
                ...
                try { // connecting to the DB
                    ...
                    const result = await pool.request()
                        .input('useremail', sql.NVarChar(255),
                    ...
                    const bcrypt_result = await bcrypt.compare(accountObj.userpassword, result.recordset[0].userpassword);
    ...
    static readByEmail(accountObj) { // accountObj gets read and useremail is checked if already exists on the database
        return new Promise((resolve, reject) => {
            (async () => {
                try {
                    const pool = await sql.connect(con);
    ...
    create() {
        return new Promise((resolve, reject) => {
            (async () => { // first we check if the user already exists on the system based on the provided useremail
                try {
                    ...
                    try {
                        const hashedPassword = await bcrypt.hash(this.userpassword, salt); // initializing the bcrypt hashing.
                        const pool = await sql.connect(con); // connection to the Database.
                        const result00 = await pool.request() // recording the inputs and storing them in the database.
                            .input('username', sql.NVarChar(50), this.username)
                            .input('useremail', sql.NVarChar(255), this.useremail)
                            .input('hashedPassword', sql.NVarChar(255), hashedPassword) // password gets hashed right away
                            .query(`
                            ...
                            if(!result00.recordset[0]) throw {statusCode: 500, errorMessage: 'Something went wrong, login was not created'}

                            const accountResponse = { // returning an accountResponse in the form of userid and username
                            ...
                            const {error} = Account.validateResponse(accountResponse);
                            console.log(error);
                            if (error) throw {statusCode: 500, errorMessage: 'Corrupt user information in the database'}
