            ...
            (async () => {
                ...
                try {
                    ...
                    schedules.forEach((schedule) => {
                        ...
                        if (error) {
                            throw { errorMessage: 'Schedule.validate failed' };
                        }
        ...
        return new Promise((resolve, reject) => {
            (async () => {
                try {
                    ...
                    const pool = await sql.connect(con);
                    const resultWithIds = await pool
                        .request()
                    ...
                    if (event.length == 0) {
                        throw {
                            statusCode: 404,
                            errorMessage: `Event not found with provided eventid: ${eventid}`,
                        };
                    }
                    ...
                    if (error) {
                        throw {
                            statusCode: 500,
                            errorMessage: `Corrupt Event informaion in database, eventid: ${eventid}`,
                        };
                    }
            ...
            (async () => {
                try {
                    ...
                    if (!result00.recordset[0]) {
                        throw {
                            dberror:
                                'Something went wrong with the INSERT to the EVENT!',
                        };
                    }
            ...
            (async () => {
                try {
                    ...
                    if (!result2.recordset[0]) {
                        throw { dberror: 'Something went wrong with the DB' };
                    }
                    ...
                    personalSchedule.forEach((el) => {
                        ...
                        if (error) {
                            throw {
                                errorMessage: `Schedule.validate on new personal schedule failed`,
                            };
                        }
    ...
    static loadDashboard(userid) {
        return new Promise((resolve, reject) => {
            (async () => {
                try {
                    const pool = await sql.connect(con);
                    const resultsOfTheUser = await pool
                        .request()
